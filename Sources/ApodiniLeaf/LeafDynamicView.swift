...
public struct LeafDynamicView<Context>: ResponseTransformable where Context: Encodable {
    private let path: String
    private let renderer: LeafRenderer
    private let context: Context
    private let information: [InformationKey: String]
    
    public init(_ path: String, with renderer: LeafRenderer, context: Context, information: [InformationKey: String] = [:]) {
        self.path = path
